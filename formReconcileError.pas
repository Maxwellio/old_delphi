...
const
  ActionStr: array[TReconcileAction] of string = ({'Skip'}'Пропустить', {'Abort'}'Прервать', 'Merge',
    {'Correct'}'Скорректировать', {'Cancel'}'Отменить', {'Refresh'}'Обновить');
  UpdateKindStr: array[TUpdateKind] of string = ('Изменение', 'Вставка',
    'Удаление');
  SCaption = {'Update Error - %s'}'Ошибка обновления - %s';
  SUnchanged = {'<Unchanged>'}'<Не изменено>';
  SBinary = {'(Binary)'}'(Двоичный)';
  SAdt = '(ADT)';
  SArray = {'(Array)'}'(Массив)';
  SFieldName = {'Field Name'}'Имя поля';
  SOriginal = {'Original Value'}'Исходное значение';
  SConflict = {'Conflicting Value'}'Конфликтующее значение';
  SValue = {' Value'}' Значения';
  SNoData = {'<No Records>'}'<Нет записей>';
  SNew = {'New'}'Новая';
