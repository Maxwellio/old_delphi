...
type
  ...
  TAnimationThread = class(TThread)
   ...
   public
     constructor Create(paintsurface : TWinControl; //контрол, на котором хотим рисовать
       paintrect : TRect;          //прямоугольная область анимации
       bkColor, barcolor : TColor; //используемые цвета
       interval : integer);        //задержка(ожидание) в милисекундах между актами рисования
   end;
...
var
  ani: TAnimationThread = nil;

procedure TfrmWait.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if ani <> nil then ani.Terminate;
  ani := nil;
end;

procedure TfrmWait.FormShow(Sender: TObject);
begin
  if ani <> nil then exit;
  ani := TanimationThread.Create(self, Rect(7,34,7+330,34+10), color, clRed, {25}100);
  Sleep(300);
end;
...
procedure TfrmWait.FormPaint(Sender: TObject);
begin
  canvas.Pen.Color := clGray;
  canvas.Rectangle(6,33,6+332,33+12);
end;
